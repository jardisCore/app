...
namespace Ecommerce\Catalog\Model\Product\Repository;
...
use Throwable;
use Ecommerce\Catalog\Entity\ProductImage as ProductImageEntity;
use Ecommerce\Catalog\Entity\ProductVariant as ProductVariantEntity;
use Ecommerce\Catalog\Entity\ProductVariantPrice as ProductVariantPriceEntity;

/**
 * Additions query class for Product.
 *
 * Loads follow-up entities based on root container data.
 * Called by both root and FK query contexts.
 * Generated code - do not modify directly.
 */
class QueryProductAdditions extends EcommerceContext
{
    ...
    public function __invoke(array $rootContainer = []): array
    {
        ...
        $this->container['productVariant'] = $this->load(
            $this->productVariant()
        );
        $this->container['productVariantPrice'] = $this->load(
            $this->productVariantPrice()
        );
        $this->container['productImage'] = $this->load(
            $this->productImage()
        );
    ...
    /**
     * Query for productVariant.
     * @throws Throwable
     */
    protected function productVariant(): DbQueryBuilderInterface
    {
        return $this->query(ProductVariantEntity::class)
            ->where('product_id')->in($this->values('product', 'id'))
            ->orderBy('sort_order', 'ASC');
    }

    /**
     * Query for productVariantPrice.
     * @throws Throwable
     */
    protected function productVariantPrice(): DbQueryBuilderInterface
    {
        return $this->query(ProductVariantPriceEntity::class)
            ->where('variant_id')->in($this->values('productVariant', 'id'));
    }
     ...
     * @throws Throwable
     */
    protected function productImage(): DbQueryBuilderInterface
    {
        return $this->query(ProductImageEntity::class)
            ->where('product_id')->in($this->values('product', 'id'))
            ->orderBy('sort_order', 'ASC');
    }
